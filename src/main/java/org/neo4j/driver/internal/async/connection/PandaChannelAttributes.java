...
public final class PandaChannelAttributes {
    ...
    public static void purgeChannel(Channel channel) {
        channelMetaMap.remove(channel);
    }

    private static <T> T get(Channel channel, AttributeKey<T> key) {
        var c = channelMetaMap.get(channel);
    ...
    private static <T> void set(Channel channel, AttributeKey<T> key, T value) {
        ...
        if (c==null) {
            c = new HashMap<>();
            channelMetaMap.put(channel, c);
        }
        c.put(key, value);
    }

    private static <T> void setOnce(Channel channel, AttributeKey<T> key, T value) {
        if (channelMetaMap.get(channel)==null || get(channel, key) == null) {
            set(channel, key, value);
            return;
        }
        var existingValue = get(channel, key);

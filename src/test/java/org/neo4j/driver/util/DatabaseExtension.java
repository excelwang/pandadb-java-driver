...
//    }
    ...
    private static GenericContainer<?> setupPandaContainer(File cert, File key, Map<String, String> config) {
        var image = new ImageFromDockerfile()
                .withFileFromPath(".", FileSystems.getDefault().getPath("/home/wanghuajin/pandadb-bin"))
                .withDockerfileFromBuilder(builder ->
                        builder
                                .from("openjdk:11")
                                .copy("pandadb-server-0.4.7","pandadb-server")
                                .entryPoint("/pandadb-server/bin/pandadb start && tail -f /pandadb-server/logs/pandadb.log")
                                .expose(PANDA_PORT)
                                .build());
        return new GenericContainer(image).withExposedPorts(PANDA_PORT).withReuse(true);
    }
